...
class Binomial_Heap{
    ...
    public : 
        ...
        BTNode*             ExtractMin();
        void                Traverse();
        inline  int         FindMin(){if(min_pointer != NULL){return min_pointer->key;}else{std::cout<<"Error : The Binomial_Heap is empty."<<std::endl;return -1;}};
        void                InsertArbitrary(const int key);
        void                InsertArbitrary(BTNode* const inserted_node);
        void                Union(Binomial_Heap &H_in);
        inline  BTNode*     GetHeadRootList(){return head_root_list;};
